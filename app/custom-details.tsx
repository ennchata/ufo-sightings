...
const CustomDetails = () => {
  ...
  useEffect(() => {
    const fetchSighting = async () => {
      try {
        const sightings: string | null = await AsyncStorage.getItem('sightings');
        if (!sightings) return;

        const data: UfoSighting[] = JSON.parse(sightings);
        const sighting = data.find((sighting) => sighting.id == parseInt(Array.isArray(id) ? id[0] : id));

        if (sighting) {
          const locationString = await getCityAndCountry(sighting.location.latitude, sighting.location.longitude);
          setSighting({ ...sighting, locationString });
        }
      } catch (error) {
